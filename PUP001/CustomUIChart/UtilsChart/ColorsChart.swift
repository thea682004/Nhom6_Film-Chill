...
open class ColorsChart: NSObject
{
    ...
    {
        let leftCharacterSet: CharacterSet = CharacterSet(charactersIn: "( ")
        let commandCharacterSet: CharacterSet = CharacterSet(charactersIn: ", ")
        
        let stringColor = colorString.lowercased()
        
        if stringColor.hasPrefix("#")
        {
            var argb: [UInt] = [255, 0, 0, 0]
            let stringColor = stringColor.unicodeScalars.dropFirst()
            let length = stringColor.count
            var index = stringColor.startIndex
            let endIndex = stringColor.endIndex
            ...
            {
                var c = stringColor[index]
                index = stringColor.index(after: index)
        ...
        else if stringColor.hasPrefix("rgba")
        {
            var a: Float = 1.0
            var r: Int32 = 0
            var g: Int32 = 0
            var b: Int32 = 0
            let scanner: Scanner = Scanner(string: stringColor)
            scanner.scanString("rgba", into: nil)
            scanner.scanCharacters(from: leftCharacterSet, into: nil)
            scanner.scanInt32(&r)
            scanner.scanCharacters(from: commandCharacterSet, into: nil)
            scanner.scanInt32(&g)
            scanner.scanCharacters(from: commandCharacterSet, into: nil)
            scanner.scanInt32(&b)
            scanner.scanCharacters(from: commandCharacterSet, into: nil)
        ...
        else if stringColor.hasPrefix("argb")
        {
            var a: Float = 1.0
            var r: Int32 = 0
            var g: Int32 = 0
        ...
        else if stringColor.hasPrefix("rgb")
        {
            var r: Int32 = 0
            var g: Int32 = 0
            var b: Int32 = 0
